...
namespace ZXing.Mobile
{
    public sealed partial class ZXingScannerControl : UserControl, IScannerView, IScannerSessionHost, IDisposable
    {
        ...
        public async void StartScanning(Action<ZXing.Result, byte[]> scanCallback, MobileBarcodeScanningOptions options = null)
        ...
        {
            await StartScanningAsync((result, imagesBytes) => scanCallback(result), options);
        }

        public async Task StartScanningAsync(Action<ZXing.Result, byte[]> scanCallback, MobileBarcodeScanningOptions options = null)
        {
            if (stopping)
            {
            ...
            timerPreview = new Timer(async (state) =>
            {
                ...
                {
                    // Get preview 
                    frame = await mediaCapture.GetPreviewFrameAsync(videoFrame);
                ...
                if (result != null && !string.IsNullOrEmpty(result.Text))
                {
                    ...
                    if (frame != null)
                    {
                        imageBytes = await EncodedBytes(frame.SoftwareBitmap, BitmapEncoder.BmpEncoderId);
                    }
                    ...
                    ScanCallback(result, imageBytes);
                }
        ...
        async Task<byte[]> EncodedBytes(SoftwareBitmap soft, Guid encoderId)
        {
            byte[] array = null;
        ...
        public Action<Result, byte[]> ScanCallback { get; set; }

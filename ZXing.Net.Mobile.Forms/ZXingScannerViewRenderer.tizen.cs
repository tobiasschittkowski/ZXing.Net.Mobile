        ...
        protected override void OnElementChanged(ElementChangedEventArgs<ZXingScannerView> e)
        {
            ...
            if (zxingWindow == null)
            {
                ...
                if (formsView.IsScanning)
                    zxingWindow.StartScanning((result, rawImage) => formsView.RaiseScanResult(result, rawImage), formsView.Options);
            ...
            if (zxingWindow != null)
            {
                switch (e.PropertyName)
                {
                    ...
                    case nameof(ZXingScannerView.IsScanning):
                        if (formsView.IsScanning)
                            zxingWindow.StartScanning((result) => formsView.RaiseScanResult(result), formsView.Options);
                        else
                            zxingWindow.StopScanning();
                        break;
